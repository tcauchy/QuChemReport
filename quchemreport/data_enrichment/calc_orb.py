...
import numpy as np
from quchemreport.utility_services.units import A_to_a0, ea0_to_D, pm_to_a0, Eh_to_eV, eA_to_D
from quchemreport.utility_services.parameters import obk_step

## Patched version of orbkit.read to read a json
from quchemreport.data_enrichment import json2orbkit
...
## Calculations
def MO(data_model: LogData, MO_list, spin="none", grid_step=obk_step, nproc=4):
    ...
    ** Returns **
    ...
    qc = json2orbkit.convert_data(data_model, all_mo=True)
...
def TD(data_model: LogData, transitions, grid_step=obk_step, nproc=4, verbose=False):
    ...
    ** Returns ** 
    ...
    MOs_alpha, X, Y, Z = MO(data_model, MO_list_alpha, spin="alpha", grid_step=grid_step, nproc=nproc)
    # Treat beta orbitals 
    if len(MO_list_beta) > 0:
        MOs_beta, X, Y, Z = MO(data_model, MO_list_beta, spin="beta", grid_step=grid_step, nproc=nproc)
...
def Potential(data_model: LogData, grid_step=obk_step, nproc=4):
    ...
    ** Returns **
    ...
    # =======================================================
    # TODO : TO IMPROVE : Compute these data only once in data enrichment
    qc = json2orbkit.convert_data(data_model)
    ...
    nb_e = np.sum(data_model.molecule.atoms_Z) - data_model.molecule.charge
    rho_n = (nb_e * rho) / (np.sum(rho)*d3r)
    rho = rho_n

    ## TODO : rho_compute fonction à part. Garde la data. ! mémoire. 
    # La partie potentiel à part pour MEP
    # Réécrire Fukui pour Garder les opé entre rho et utiliser un appel vers calcul_rho. 
...
#                          grid.min_[2] *2 : grid.max_[2] *2 : dz]
    ...
    ## 4. Convolute
    ## Classic approach. " TODO : proposer approche classique autre fonction et choix dans Yaml."
#    for i in range(TIx):
#        ie = i + TIx
#        for j in range(TIy):
#            je = j + TIy
...
#                V_e_bis[i,j,k] += (rho_r * R[i:ie, j:je, k:k + TIz]).sum()
    ...
    return rho, MEP, X, Y, Z, rho

def Fukui(data_model_opt: LogData, data_model_sp: LogData, precomputed_rho_opt=None, label=None, grid_step=obk_step, nproc=4):
    ...
    ** Parameters **
    ...
    qc_opt = json2orbkit.convert_data(data_model_opt) # déjà fait si MEP ! 
    qc_sp = json2orbkit.convert_data(data_model_sp) # même chose que rho ref mais avec autre état. 

    X, Y, Z = _init_ORB_grid(qc_opt, grid_step=grid_step) # vérifier que l'on a la même ! 

    dx, dy, dz = (X[10,0,0] - X[9,0,0]), (Y[0,10,0] - Y[0,9,0]), (Z[0,0,10] - Z[0,0,9])
    d3r = dx*dy*dz

    # If rho_opt was not precomputed we compute it
    if precomputed_rho_opt is None:
      rho_opt = core.rho_compute(qc_opt, numproc=nproc)
    else:
      rho_opt = precomputed_rho_opt
    
    rho_sp = core.rho_compute(qc_sp, numproc=nproc)
    
    charge_diff = data_model_opt.molecule.charge - data_model_sp.molecule.charge
    print ("OPT and SP charges and diff", data_model_opt.molecule.charge , data_model_sp.molecule.charge, charge_diff )
    ...
    return delta_rho, X, Y, Z, rho_opt


def Fdual(data_model_opt: LogData, delta_rho_plus, delta_rho_minus, grid_step=obk_step):

    qc_opt = json2orbkit.convert_data(data_model_opt)
...
def CDFT_plus_Indices(data_model_opt: LogData, data_model_spplus: LogData):
    Eref = data_model_opt.results.wavefunction.total_molecular_energy
    Eplus = data_model_spplus.results.wavefunction.total_molecular_energy
    Mulliken_partial_charges_opt = data_model_opt.results.wavefunction.Mulliken_partial_charges
    Hirshfeld_partial_charges_opt = data_model_opt.results.wavefunction.Hirshfeld_partial_charges
    Hirshfeld_partial_charges_spplus = data_model_spplus.results.wavefunction.Hirshfeld_partial_charges
    
    # Check if all necessary data is available
    if (
      Eref is None
      or Eplus is None
      or Mulliken_partial_charges_opt is None
    ): 
      return None, None, None

    A = (Eref-Eplus) / Eh_to_eV

    OPT_charges = np.array(Mulliken_partial_charges_opt)
    SPplus_charges = np.array(Mulliken_partial_charges_opt)
    fplus_lambda_mulliken = np.subtract(OPT_charges, SPplus_charges)

    if Hirshfeld_partial_charges_opt != None:
      OPT_charges = np.array(Hirshfeld_partial_charges_opt)
    else:
      OPT_charges = []
    
    if Hirshfeld_partial_charges_spplus != None:
      SPplus_charges = np.array(Hirshfeld_partial_charges_spplus)
    else:
      SPplus_charges = []
...
def CDFT_minus_Indices(data_model_opt: LogData, data_model_spminus: LogData):
    Eref = data_model_opt.results.wavefunction.total_molecular_energy
    Eminus = data_model_spminus.results.wavefunction.total_molecular_energy
    Mulliken_partial_charges_opt = data_model_opt.results.wavefunction.Mulliken_partial_charges
    Mulliken_partial_charges_spminus = data_model_spminus.results.wavefunction.Mulliken_partial_charges
    Hirshfeld_partial_charges_opt = data_model_opt.results.wavefunction.Hirshfeld_partial_charges
    Hirshfeld_partial_charges_spminus = data_model_spminus.results.wavefunction.Hirshfeld_partial_charges
    ...
    if (
      ...
      or Eminus is None
      or Mulliken_partial_charges_opt is None
      or Mulliken_partial_charges_spminus is None
    ):
      return None, None, None
    ...
    OPT_charges = np.array(Mulliken_partial_charges_opt)
    SPminus_charges = np.array(Mulliken_partial_charges_spminus)
    ...
    if Hirshfeld_partial_charges_spminus != None:
      SPminus_charges = np.array(Hirshfeld_partial_charges_spminus)
    else:
      SPminus_charges = []
...
def CDFT_Indices(data_model_opt: LogData, data_model_spplus: LogData, data_model_spminus: LogData):
    ...
    if data_model_opt is None:
      return None, None, None, None, None, None, None, None, None, None, None, None

    # Compute the CDFT Plus Indices
    A = None
    fplus_lambda_mulliken = None
    fplus_lambda_hirshfeld = None
    if data_model_spplus is not None:
      A, fplus_lambda_mulliken, fplus_lambda_hirshfeld = CDFT_plus_Indices(data_model_opt, data_model_spplus)
    
    # Compute the CDFT Minus Indices
    I = None
    fminus_lambda_mulliken = None
    fminus_lambda_hirshfeld = None
    if data_model_spminus is not None:
      I, fminus_lambda_mulliken, fminus_lambda_hirshfeld = CDFT_minus_Indices(data_model_opt, data_model_spminus)
    ...
    has_dual = (
      # CDFT Plus Indices available
      (
        A is not None
        and fplus_lambda_mulliken is not None
        and fplus_lambda_hirshfeld is not None
      )
      # CDFT Minus Indices available
      and (
        I is not None
        and fminus_lambda_mulliken is not None
        and fminus_lambda_hirshfeld is not None
      )
    )
    ...
    Omega = None
    DeltaN = None
    fdual_lambda_mulliken = None
    fdual_lambda_hirshfeld = None
